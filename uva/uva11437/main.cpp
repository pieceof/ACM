...
point_t pa,pb,pc,pe,pf,pd,pr,pp,pq;
int main() {
...
//    freopen("out.txt", "w", stdout);
    int t; scanf("%d",&t); while ( t-- ){
        ...
        pq = lineCrossline(line_t(pf,pc),line_t(pb,pe));
        pr = lineCrossline(line_t(pa,pd),line_t(pf,pc));
        pp = lineCrossline(line_t(pb,pe),line_t(pa,pd));

        double ans = TriangleArea(pq,pr,pp);
        printf("%.0f\n",ans);
    }
