...
int main() {
    scanf("%d",&t);while ( t-- ){
        ...
        l[0] = line_t ( point_t(t1,s2),point_t(t1,s1) );
        l[1] = line_t ( point_t(t2,s1),point_t(t2,s2) );
        l[2] = line_t ( point_t(t1,s1),point_t(t2,s1) );
        l[3] = line_t ( point_t(t2,s2),point_t(t1,s2) );
        l[4] = line_t ( point_t(0,-w),point_t(w,0) );
        l[5] = line_t ( point_t(0,w),point_t(-w,0) );
        n = HalfplaneIntersection(l,6,p);
//        deBug(n);
