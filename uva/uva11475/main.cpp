...
//    while( scanf("%d",&k)!=EOF &&k ){
    while ( scanf("%s",str2) != EOF ){
        int mid = strlen(str2);
        strcpy(str,str2);
        reverse(str2,str2+mid);
        strcpy(str+mid+1,str2);
        str[mid] = 'Z';
        int len = strlen(str);
//        deBug(str);
...
//        if ( kase++ != 1 )puts("");
        ans=0;
//        for (int i = 0;i < len;++i ){
//            if ( Height[i] > ans && Height[i]+min(SA[i],SA[i-1]) ==mid  && (min(SA[i],SA[i-1]) < mid) && (max(SA[i],SA[i-1]) > mid)  ){
//                ans = Height[i];
//            }
//        }
        int j = 0;
        ...
        int j1 = j+1;
        while( j1 < len ){
            minpi = min(minpi,Height[j1]);
            if ( mid-SA[j1] == minpi ) break;
            j1++;
        }
        ans = minpi;
//        deBug(minpi);
//        j--;
        minpi = INT_MAX;
        while( j > 0  ){
            minpi = min(minpi,Height[j]);
            if ( mid-SA[j] == minpi ) break;
            j--;
        }
        ans = max(ans,minpi);
//        deBug(ans);
