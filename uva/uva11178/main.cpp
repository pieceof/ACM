...
point_t getd( point_t const&a,point_t const&b,point_t const&c ){
    vector_t bc = c-b;
    vector_t ba = a-b;
    vector_t cb = b-c;
    vector_t ca = a-c;
    double rad1 = vector_angle(bc,ba);
    vector_t bd = vector_rotate( bc,fabs(rad1)/3 );
    double rad2 = vector_angle(ca,cb);
    vector_t cd = vector_rotate( cb,-fabs(rad2)/3 );
    point_t d = lineCrossline( b,bd,c,cd);
    return d;
}
...
int main()
{
    ...
    int t;scanf("%d",&t);while ( t-- ){
        ...
        point_t a(x1,y1),b(x2,y2),c(x3,y3);

        printf("%.6f %.6f ",getd(a,b,c).x,getd(a,b,c).y);
        printf("%.6f %.6f ",getd(b,c,a).x,getd(b,c,a).y);
        printf("%.6f %.6f\n",getd(c,a,b).x,getd(c,a,b).y);
