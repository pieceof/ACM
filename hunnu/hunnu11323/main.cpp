...
/**< RoundIntersectArea   */
double RIA(round_t a, round_t b) {
...
void Rotating_calipers( point_t *p ,int n ){
    int q = 1;
    double ans = 0;
    p[n] = p[0];
    point_t p1,p2;
...
//            deBug(p[i+1]);deBug(p[q+1]);
            if ( ans < Point2Point(p[i],p[q]) ) {
                ans = Point2Point(p[i],p[q]);
                p1 = p[i],p2 = p[q];
            }
            if ( ans <  Point2Point(p[i+1],p[q+1]) ) {
                ans = Point2Point(p[i+1],p[q+1]);
                p1 = p[i+1],p2 = p[q+1];
            }
//            ans = max( ans ,max( Point2Point(p[i],p[q]), Point2Point(p[i+1],p[q+1]) ) );
    }
    printf("Case %d: %d %d\n",kase++, min ( p1.id,p2.id) ,max( p1.id,p2.id ));
//    return ans;
}
