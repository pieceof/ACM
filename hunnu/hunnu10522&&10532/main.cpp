...
//        scanf("%d",&n);
    segment_t s1,s2;
    while ( scanf("%lf%lf%lf%lf",&s1.s.x,&s1.s.y,&s1.e.x,&s1.e.y) != EOF ){
        scanf("%lf%lf%lf%lf",&s2.s.x,&s2.s.y,&s2.e.x,&s2.e.y);

        point_t po(0,0);
        if ( !SegmentOnSegment(s1,s2) ){
            puts("None");
        }else {
//            deBug(cross(po, s1.s-s1.e,s2.s-s2.e) );
            if (  dcmp( cross(po, s1.s-s1.e,s2.s-s2.e)) == 0){
                if ( s1.e<s1.s ) swap( s1.s,s1.e);
                if ( s2.e<s2.s ) swap( s2.s,s2.e);
                if ( s1.e != s2.s && s1.s != s2.e)
                    puts("Many");
                else puts("One");
