...
//        scanf("%d",&n);
    ...
    while ( gets(str) ){
        ...
        circle_t C = CircumscribedCircle(p[0],p[1],p[2]);
        double a = vector_angle(C.p-p[0],C.p-p[1]);
        double b = vector_angle(C.p-p[1],C.p-p[2]);
        double c = vector_angle(C.p-p[2],C.p-p[0]);
//        a = fabs(a);
//        b = fabs(b);
//        c = fabs(c);
//        assert(a>0);
...
//            deBug( b/a0);
//            deBug( c/a0);
            if ( check(a/a0) && check(b/a0)&&check(c/a0) ){
                isok = 1;
                printf("%d\n",i);
                break;
            }
