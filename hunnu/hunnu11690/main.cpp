...
//int str_cnt;
//int get_str_id( string const&str ){
//    if (str_id.count(str) ) return str_id[str];
//    str_cache.push_back(str);
//    return str_id[str]=str_cnt ++;
//}
//int father[SIZE];
//int  find ( int x ){
...
segment_t sg[SIZE],sg1[SIZE],sg2[SIZE];
line_t l[SIZE],l1[SIZE],l2[SIZE];
//char str[1000];
//bool check (double x ){
//    return dcmp( x - int(x+0.5) )== 0 ;
//}
int main() {
...
//        scanf("%d",&n);
    segment_t s1,s2;
    while ( scanf("%lf%lf%lf%lf%lf%lf%lf%lf",&s1.s.x,&s1.s.y,&s1.e.x,&s1.e.y
                  ,&s2.s.x,&s2.s.y,&s2.e.x,&s2.e.y) != EOF ){
        bool isok = 1;
//        if ( PointOnSegment(s1,s2.s) && PointOnSegment(s1,s2.e) ) isok = 0;
//        if ( PointOnSegment(s2,s1.s) && PointOnSegment(s2,s1.e) ) isok = 0;
//isok = 0;

        if ( s1.e < s1.s   )swap( s1.s,s1.e );
        if ( s2.e < s2.s   )swap( s2.s,s2.e );
//        deBug(SegmentOnSegment(s1,s2));
        if ( SegmentOnSegment(s1,s2) ) isok = 0;
//        deBug( vector_angle(s1.s-s1.e , s2.s-s2.e) );
//        deBug(  Segment2Segment(s1,s2)  );
        if ( dcmp( vector_angle(s1.s-s1.e , s2.s-s2.e) ) == 0

            && dcmp( Segment2Segment(s1,s2) ) == 0 ){
            if ( s2.e == s1.s || s1.s == s2.e )isok = 1;
            else isok = 0;
        }
