...
//        n = Graham(p,n);
        double ans = PolygonArea(p,n);
        double ans2 = PolygonArea(p1,n);
//        deBug(ans2);
//        deBug(ans);
        if ( kase != 1)puts("");
        ...
        p[n] = p[0];
        for (int i = 0 ;i < n;++i ){
            sg[i] = segment_t(p[i],p[i+1]);
        }
        for (int i = 0 ;i < n-1;++i ){
            for (int j = 0;j < i-1;++j ){
                if(SegmentOnSegment(sg[i],sg[j])) iscross = 1;
            }
        }
        for (int i = 1 ;i < n-2;++i){
            if ( SegmentOnSegment(sg[n-1],sg[i]))iscross = 1;
        }
//        if ( )
        if ( n < 3 || iscross  ){
            puts("Impossible");
