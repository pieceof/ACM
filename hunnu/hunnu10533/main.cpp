...
//        scanf("%d",&n);
    ...
    while (scanf("%d%d%d%d%d%d",&p[0].x,&p[0].y,&p[1].x,&p[1].y,&p[2].x,&p[2].y) != EOF ){
        if ( (p[0].x||p[0].y||p[1].x||p[1].y||p[2].x||p[2].y) == 0 )break;
        double area = fabs(PolygonArea(p,3));
//        deBug(area);
//        printf("%.3f\n",PI*(c1.r*c1.r+c2.r*c2.r)-ans);
        int cnt1 = gcd( abs(p[0].x - p[1].x),abs(p[0].y - p[1].y)  );
        int cnt2 = gcd(abs(p[2].x - p[1].x) ,abs(p[2].y - p[1].y)  );
        int cnt3 = gcd( abs(p[2].x - p[0].x), abs(p[2].y - p[0].y) );
 //*  面积 = 边界点数 ÷ 2 ＋ 内部点数 － 1
        int ans = area*2+2-cnt1-cnt2-cnt3;
        printf("%d\n",( ans ) / 2);
