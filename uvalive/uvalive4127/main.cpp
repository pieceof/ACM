...
struct point_t{
    ...
    bool operator == (  point_t const&a )const {return dcmp(a.x-x) == 0;}
    bool operator != (  point_t const&a )const {return ! (*this == a ); }
    bool operator <  (  point_t const&a )const {return x < a.x ;}
...
//    int t; scanf("%d",&t); while ( t-- ){
    ...
    while ( scanf("%d",&n) != EOF && n ){
        ...
        for (int i = 0;i < n;++i ){
            ...
            // 左端点
            p[pcnt].x = x - b*0.5 , p[pcnt].y = 0;
            sg[scnt].s = p[pcnt++];
            // 顶峰
            p[pcnt].x = x , p[pcnt].y = h;
            sg[scnt++].e = p[pcnt];
            sg[scnt].s = p[pcnt++];
            // 右端点
            p[pcnt].x = x + b*0.5 , p[pcnt].y = 0;
        ...
        for (int i = 0 ;i < scnt;++i ){
            for (int j = i+1;j < scnt;++j ){
                if ( isParallel( line_t(sg[i].s,sg[i].e),line_t(sg[j].s,sg[j].e) ) ) continue;
                if ( SegmentOnSegment(sg[i],sg[j]) )
                    p[pcnt++] = lineCrossline( line_t(sg[i].s,sg[i].e),line_t(sg[j].s,sg[j].e) );
            }
        }
        ...
        for ( int i = 0;i < pcnt;++i ){
            for (int j = 0;j < scnt;++j ){
                if (  sg[j].s.x <= p[i].x && p[i].x <= sg[j].e.x ){

                    point_t pc = lineCrossline( line_t( point_t(p[i].x,0),point_t(p[i].x,100) )
                                ,line_t( sg[j].s,sg[j].e ) );
                    p[i].y = max( p[i].y , pc.y );
                }
            }
        }
//        for (int i = 0;i < pcnt ;++i ) cout << p[i] << endl; cout << endl;
        for (int i = 0;i < pcnt-1;++i )
            if ( dcmp(p[i].y) > 0 || dcmp(p[i+1].y) > 0 )
                ans += Point2Point(p[i],p[i+1]);
//        Case;
        printf("Case %d: %.0f\n\n",kase++,ans);
    }
