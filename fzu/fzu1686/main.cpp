...
//    if ( t-- == 0 )return false;
    ...
    for (int i = 0;i < n ;++i )for (int j = 0;j < m;++j){
        scanf("%d",mat[i]+j);
        if ( mat[i][j] ) mat[i][j] = ++cnt;
    }
    int lc,rc;
    scanf("%d%d",&rc,&lc);
    init((n-rc+1)*(m-lc+1),cnt);
    int attack = 1;
    int tmp;
    for (int i = 0;i <= n-rc;++i )for (int j = 0;j <= m-lc;++j ){

        for (int ri = 0;ri < rc;ri++ )for (int lj = 0;lj < lc;++lj)

            if ( tmp = mat[i+ri][j+lj] ) link( attack ,tmp );
        attack++;
    }
//    disp("r");
