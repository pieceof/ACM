...
int Section_min[SIZE];
int Section_max[SIZE];
void init_Section(){
    fill(Section_min,Section_min+20,INT_MAX);
    fill(Section_max,Section_max+20,INT_MIN);
}
vector <int> ans_pos;
bool check( int len ,int mid){
    ...
    for(int i = 0;i < len;++i ){
        if ( Height[i] >= mid ){
            int sec1 = chin[SA[i-1]];
            int sec2 = chin[SA[i]];

            Section_max[sec1] = max(Section_max[sec1],SA[i-1]);
            Section_min[sec1] = min(Section_min[sec1],SA[i-1]);
            if ( Section_max[sec1] - Section_min[sec1] >= mid )
                if ( !vis[sec1] )vis[sec1]=1,viscnt++;

            Section_max[sec2] = max(Section_max[sec2],SA[i]);
            Section_min[sec2] = min(Section_min[sec2],SA[i]);
            if ( Section_max[sec2] - Section_min[sec2] >= mid )
                if ( !vis[sec2] )vis[sec2]=1,viscnt++;
