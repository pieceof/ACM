...
//    int base = (r*allSize+c)*allSize+v;
    ...
    link(base,
         (Ncnt - 1) * allSize + v + 2 * allSize * allSize);     //deBug( base ,(Ncnt-1)*allSize+v + 2*allSize*allSize );
 ...
 */
void sudoku(int const mat[], int output[20][20]) {
    base = 1;
    gridCnt = 1;
    init(allSize * allSize * allSize, allSize * allSize * 4);
    for (int i = 0, pos = 0; i < allSize; ++i)
        for (int j = 0; j < allSize; ++j, pos++, gridCnt++) {
            if (0 == mat[pos])
                // 这个点尝试放置 1~9
                for (int k = 1; k <= allSize; ++k)
                    _coverPos(i, j, k);
            else
                _coverPos(i, j, mat[pos]);
        }
    int np = dlx(0);
//    cout << np << endl;
