//
// Created by bitwater on 18-4-20.
//
...
void update(point_t _p1, point_t _p2, vector_t _v) {
    ans = min(ans, Point2Segment(_p2, _p2 + _v, _p1));
}
void solve(int n1, int n2) {
    int t1 = 1, t2 = 1;
    point_t pp1(p1[0]), pp2(p2[0]);

    while (t1 < n1 && t2 < n2) {
        vector_t v1 = p1[t1] - pp1;
        vector_t v2 = p2[t2] - pp2;
        double len1 = vector_length(v1);
        double len2 = vector_length(v2);
        double len = min(len1, len2);
        update(pp1, pp2, v2 * len / len2 - v1 * len / len1);
        pp1 = pp1 + v1 * len / len1;
        pp2 = pp2 + v2 * len / len2;
        if (pp1 == p1[t1])t1++;
        if (pp2 == p2[t2])t2++;
