...
int main(){
    int t;scanf("%d",&t);while ( t-- ){
        ...
        if ( cross(O,p,q) > 0 )swap(p,q);
        vector_t op = p;
        vector_t oq = q;
        double angle = vector_angle(oq,op);
        vector_t od = vector_rotate(oq,angle/2);
        point_t d = od/vector_length(od) * r ;
        double ans = Point2Point(d,p)+Point2Point(d,q);
        double l = vector_length(op);
        point_t p1 = op/l*r*r/l;
        point_t q1 = oq/l*r*r/l;
        if ( Point2Line(O,line_t(p1,q1)) <= r)
            ans = min(ans , Point2Point(p1,q1) * l / r );
        printf("%.10f\n",ans);
