...
//        getExtendKMPNext(part,EkmpNext,strlen(part));

//        printf("Case #%d: %d\n",kase++,ans);

//        dispArray(extend,strlen(T));
//        dispArray(Next,strlen(part));
//        dispArray(EkmpNext,len);
        strcpy(str,part);
        manacher(part,len);

//        dispArray(p,strlen(part)+1);
        int l,r,mx=0;
        len = strlen(part);
//        puts(part);
        for (int i = 1;i < len;++i ){
            if ( mx < p[i]-1 )mx = p[i]-1,l=i;
        }
        if ( mx == 1 )puts("No solution!");
        else {

            l = (l-2)/2-mx/2;if ( mx % 2 == 0 )l++;
            printf("%d %d\n",l,l+mx-1);
            for (int i = l;i <= l+mx-1;++i ){
                char c = int(str[i]-'a'-ch[0]+'a'+26)%26+'a';
                printf("%c", c );
            }
            printf("\n");
        }
