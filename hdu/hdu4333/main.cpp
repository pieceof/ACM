...
//        deBug(part);
//        deBug(tar);
        getExtendKMP(part,strlen(part),tar,strlen(tar),EkmpNext,extend);
        getNext(part,Next,strlen(part));
//        dispArray(extend,strlen(tar));
//        dispArray(Next,strlen(part));
//        dispArray(EkmpNext,strlen(part));
...
//        deBug(cnt);
        int k = 1;
        if ( len % cnt == 0 ) k = len / cnt;
//        deBug(k);
        for (int i = 0; i < len; ++i ) {
            if ( extend[i] >= len ) e++;
            if ( tar[extend[i]] > tar[extend[i]+i] )l++;
            if ( tar[extend[i]] < tar[extend[i]+i] )r++;
        }
        printf("Case %d: %d %d %d\n",kase++,l/k,e/k,r/k);
//        deBug(e);
//        deBug(l);
