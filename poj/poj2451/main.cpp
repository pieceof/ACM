...
segment_t sg[SIZE];
point_t p[SIZE*4],p1[SIZE],p2[SIZE];
line_t l[SIZE];
...
//	scanf("%d",&t);
	while(  scanf("%d",&n) != EOF ){
        ...
        l[n++] = line_t( point_t(0,0) ,point_t(10000,0) );
        l[n++] = line_t( point_t(10000,0) ,point_t(10000,10000) );
        l[n++] = line_t( point_t(10000,10000) ,point_t(0,10000) );
        l[n++] = line_t( point_t(0,10000) ,point_t(0,0) );
        int m = HalfplaneIntersection(l,n,p);
//        m = Graham(p,m);
        double ans = PolygonArea(p,m);
        printf("%.1f\n",ans);
	}
	return 0;
}
