...
{//point_t a,vector_t v,point_t c,vector_t w
    point_t  p0(0,0);
    vector_t v = l1.b - l1.a;
    vector_t w = l2.b - l2.a;
    vector_t u = l1.a - l2.a;
    int_t t = cross( p0,w,u ) / cross( p0,v,w );
    return point_t( l1.a.x + v.x*t, l1.a.y + v.y*t );
}
...
line_t l[SIZE];
vector_t v[SIZE],vf[SIZE];
circle_t c[SIZE];
int kase = 1,n,n1,n2,t;
point_t s;
int main(){
...
//            if ( kase++ > 1 ) puts("");
        scanf("%lf%lf",&s.x,&s.y);
        line_t floor( point_t(0,0),point_t(1,0) );
        for (int i = 0;i < n;++i )
            scanf("%lf%lf%lf",&c[i].p.x,&c[i].p.y,&c[i].r);
        for (int i = 0;i < n;++i ){
            assert(dcmp(c[i].r) != 0);
//            if ( dcmp(c[i].r) == 0 ) while(1);
            int cn = LineTangentCircle(s,c[i],v);
            assert( cn == 2 );
            p2[0] = lineCrossline( line_t(s,s+v[0]),floor );
            p2[1] = lineCrossline( line_t(s,s+v[1]),floor );
            if ( p2[1] < p2[0]  ) swap( p2[0],p2[1] );
            sg[i] = segment_t(p2[0],p2[1]);
        }
...
//            point_t tmp = sg[i].e;
            int j = i+1;
            double maxx = sg[i].e.x;
            while ( j < n&&( sg[j].s.x <= maxx ) ){
                maxx = max(maxx,sg[j++].e.x);
//                j++;
            }
//            ans1 = Point2Point( point_t(0,0),sg[i].s);
//            ans2 = Point2Point( point_t(0,0),sg[j-1].e );
                ans1 = sg[i].s.x;
                ans2 = maxx;
//            deBug(ans);
//            double t1 = point
