...
//        int n2 = Graham(p,n);
        p[n] = p[0];
        for (int i = 0; i < n;++i ){
            l1[i] = line_t( p[i],p[i+1] );
        }
        for (int i = n;i > 0;--i ){
            l2[i-1] = line_t( p[i],p[i-1] );
        }
        int n1 = HalfplaneIntersection(l1,n,p);
//        double ans1 = PolygonArea(p,n1);
        int n2 = HalfplaneIntersection(l2,n,p);
////        double ans2 = PolygonArea(p,n2);
