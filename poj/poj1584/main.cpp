...
bool PointInSimple( point_t * p ,int n , point_t const&po ){
    ...
    p[n] = p[0];
    for (int i = 0;i < n;++i ){
        if ( PointOnSegment(po,p[i],p[i+1])) return 1;
...
//        int n2 = Graham(p,n);
        int shunxu ;
        if ( isConvexPolygon(p,n,shunxu) == 0 )
            printf("HOLE IS ILL-FORMED\n");

        else {
//            n =
            double dis = Point2Simple(p,n,c.p);
//            deBug(shunxu);
////            if ( shunxu<0 ){
...
////                }
////            }
//            deBug( PointInSimple(p,n,c.p));

            if( PointInSimple(p,n,c.p) && dcmp( dis-c.r ) >= 0 )
                printf("PEG WILL FIT\n");
            else
