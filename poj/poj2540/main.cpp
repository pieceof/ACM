...
int main() {
    point_t last(0,0);
    point_t now;
    l[0] = line_t( point_t(0,0),point_t(10,0) );
    l[1] = line_t( point_t(10,0),point_t(10,10) );
    l[2] = line_t( point_t(10,10),point_t(0,10) );
    l[3] = line_t( point_t(0,10),point_t(0,0) );
    n = 4;
    double ans = 1e90;
    while ( scanf("%lf %lf %s",&now.x,&now.y,str) != EOF ){
        ...
        point_t mid = SegmentMid(now,last);
        vector_t v = now-last;
        vector_t vf = vector_normal(v);
        if ( *str == 'H' )vf = vf*-1;

        l[n++] = line_t(mid,mid+vf);
        sort( l,l+n );
//        deBug(n);
        int tn = HalfplaneIntersection(l,n,p);
        double ans = PolygonArea(p,tn);
//        deBug(ans);
        printf("%.2f\n",ans);
        last = now;
    }
