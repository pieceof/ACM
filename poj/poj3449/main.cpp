...
bool isCross( vector<point_t> const&p1,vector<point_t> const&p2){
    for (int i = 0;i < p1.size()-1;++i ){
        for (int j = 0 ; j < p2.size()-1;++j ){
//                SegmentOnSegment()
            if ( SegmentOnSegment( segment_t(p1[i],p1[i+1]) , segment_t(p2[j],p2[j+1])  ) ) return true;
        }
    }
    return false;
}
...
void read( char s,vector<point_t> &vp ){
    vp.clear();
    int sn = 0;
    if ( s == 'p' ){
        scanf(" %d",&sn);
        for (int i = 0 ;i < sn;++i ){
            scanf(" (%lf,%lf)",&ptmp[i].x,&ptmp[i].y );
            vp.push_back(ptmp[i]);
        }
        vp.push_back(ptmp[0]);
...
//        x4:=(x1+x3-y3+y1)/2; y4:=(y1+y3-x1+x3)/2;
        vp.push_back(ptmp[0]);
        vp.push_back(point_t( (ptmp[0].x+ptmp[1].x+ptmp[1].y-ptmp[0].y)/2, (ptmp[0].y+ptmp[1].y+ptmp[0].x-ptmp[1].x)/2 ) );
        vp.push_back(ptmp[1]);
        vp.push_back(point_t( (ptmp[0].x+ptmp[1].x-ptmp[1].y+ptmp[0].y)/2, (ptmp[0].y+ptmp[1].y-ptmp[0].x+ptmp[1].x)/2 ) );
        vp.push_back(ptmp[0]);
        ...
        vp.push_back(point_t( ptmp[0].x+ptmp[2].x-ptmp[1].x,ptmp[0].y+ptmp[2].y-ptmp[1].y ) );
        vp.push_back(ptmp[0]);
...
//void printp( vector<point_t>pppp ){
//    for ( auto i : pppp ){
//        cout << "(" << i.x << "," << i.y << ")" << " " ;
//    }
//    cout << endl;
...
//        cout << "cal" << endl;
        typedef map<char, vector<char> >::iterator m_c_vc_iter;
        typedef map<char,vector<point_t> >::iterator m_c_vp_iter;
//        for ( auto i : poly ){
//
//            for ( auto j : poly )
//
//
        ans.clear();
        for ( m_c_vp_iter i = poly.begin(); i != poly.end() ;++i  )ans[i->first] = vector<char>();
        for ( m_c_vp_iter i = poly.begin(); i != poly.end() ;++i  ){
            m_c_vp_iter j = i;
//            ans[i->first] = vector<char>();
            j++;
            for ( ; j != poly.end();++j ){
//                deBug(i->first);
//                printp(i->second);
//                deBug(j->first);
//                printp(j->second);
                if ( isCross(i->second,j->second) ){
//                    cout << " cross ====== >=======<" <<endl;
//                    deBug( i->first );
//                    deBug( j->first );
//                    cout << " cross ====== >=======<" <<endl;
                    ans[i->first].push_back(j->first);
                    ans[j->first].push_back(i->first);
                }
            }
        }
//        for ( auto i : ans ){
        for ( m_c_vc_iter i = ans.begin();i != ans.end();++i ){
            cout << i->first;cout << " " ;
            if ( i->second.size() == 1 ){
                cout << "intersects with " << i->second[0] << endl;
            }else if (i->second.size() == 2) {
                cout << "intersects with " << i->second[0] << " and " << i->second[1] << endl;
            }else if ( i->second.size() == 0 ){
                cout << "has no intersections" << endl;
            }else {
//                intersects with S, W, and X
                cout << "intersects with " ;
                for ( int j = 0;j < i->second.size()-1;++j ){
                    cout << i->second[j] << ", " ;
                }
                cout << "and " << i->second[i->second.size()-1] << endl;
            }
        }
        cout << endl ;
    }
